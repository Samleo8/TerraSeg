...
# Paths.
results_root = Path("PUT_YOUR_DIRECTORY_HERE/PseudoLabeler__ablation_study2")
omnilidar_root = Path("PUT_YOUR_DIRECTORY_HERE/OmniLiDAR")

assert str(results_root) != "PUT_YOUR_DIRECTORY_HERE/PseudoLabeler__ablation_study2", (
...
for sample_dict in tqdm(dataloader, desc="Scans"):
    ...
    for combo in combinations:
        combo_folder = f"vxy_{combo['vxy']}_tau_{combo['tau']}"
        labels_path = (
            results_root
            / combo_folder
        ...
        if not labels_path.exists():
            all_exist = False
            missing_combos.append((combo, labels_path))
    if all_exist:
        continue
    ...
    with torch.no_grad():
        ...
        # Loop over missing combinations.
        for combo, labels_path in missing_combos:
            postprocessed_pred_labels = model.postprocess_recover_non_ground(
                pc=pc_lidar, pred_labels=pred_labels, vxy=combo["vxy"], tau=combo["tau"]
            )
...
for combo in combinations:
    combo_folder = f"vxy_{combo['vxy']}_tau_{combo['tau']}"
    combo_pseudolabels_root = results_root / combo_folder

    print(f"Evaluating combination: vxy = {combo['vxy']} m and tau = {combo['tau']} m")

    for dataset_name, split_name in zip(eval_splits.keys(), eval_splits.values()):
        dataset = OmniLiDARDataset(
            root=omnilidar_root,
            splits={dataset_name: split_name},
            pseudolabels_root=combo_pseudolabels_root,
            remove_ego_points=False,
            compute_scans_metadata=True,
        )
        ...
        for sample_dict in tqdm(dataloader, desc=f"Evaluating {dataset_name}"):
            ...
            tp0, fn0, fp0, tn0 = get_confusion_matrix(
                gt_labels=target_labels, pred_labels=target_pseudolabels, selected_id=0, ignore_id=2
            )
            tp1, fn1, fp1, tn1 = get_confusion_matrix(
                gt_labels=target_labels, pred_labels=target_pseudolabels, selected_id=1, ignore_id=2
            )
        ...
        metrics_dict0 = get_standard_metrics(tp=total_tp0, fn=total_fn0, fp=total_fp0, tn=total_tn0)
        metrics_dict1 = get_standard_metrics(tp=total_tp1, fn=total_fn1, fp=total_fp1, tn=total_tn1)
