...
from truckscenes.utils.splits import train
...
assert str(source_root) != "PUT_YOUR_DIRECTORY_HERE/TruckScenes", (
    "Directory to TruckScenes dataset. Change to directory in your file system!"
)
...
for seq_num, scene_name in tqdm(list(enumerate(train)), desc="Scenes"):
    ...
    for sensor_name in sensor_names:
        ...
        downsampled_lidar_tokens = get_downsampled_lidar_tokens(
            lidar_tokens=lidar_tokens, timestamps=timestamps, period=TARGET_PERIOD
        )
...
for sensor_name in sensors:
    if sensor_name in sensors:  # ['LIDAR_TOP_FRONT', 'LIDAR_TOP_LEFT', 'LIDAR_TOP_RIGHT']:
        ...
        if extra_rotation == 0:
            ...
            T_lidaraligned2_lidaraligned = np.array(
