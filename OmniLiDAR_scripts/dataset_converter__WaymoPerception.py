...
assert str(omnilidar_root) != "PUT_YOUR_DIRECTORY_HERE/OmniLiDAR", (
    "Folder for OmniLiDAR dataset. Change to directory in your file system!"
)
assert str(source_root) != "PUT_YOUR_DIRECTORY_HERE/WaymoPerception", (
    "Directory to Waymo Open Perception dataset. Change to directory in your file system!"
)
...
def convert_range_image_to_point_cloud_labels_v2(
    ...
    segmentation_label_points_tensor = tf.gather_nd(
        segmentation_label_tensor, tf.where(range_image_mask)
    )
...
context_names = sorted(
    [
        ...
        for context_dir in glob.glob(str(source_root / split_name / lidar_tag_name / "*.parquet"))
    ]
)
...
for seq_num, context_name in tqdm(enumerate(context_names), desc="Contexts"):
    ...
    timestamps_sec = (
        df_pd["key.frame_timestamp_micros"].to_numpy() * 1e-6
    ).tolist()  # Unit: seconds.
    row_nums = np.arange(len(df_pd)).tolist()  # Unit: 1.
    downsampled_row_nums = get_downsampled_row_nums(
        row_nums=row_nums, timestamps=timestamps_sec, period=TARGET_PERIOD - 1e-3
    )
    ...
    for scan_num, row_idx in enumerate(downsampled_row_nums):
        ...
        pc_lidar = v2.convert_range_image_to_point_cloud(
            range_image=lidar_data.range_image_return1,
            calibration=lidar_calib,
            keep_polar_features=False,
        ).numpy()

        raw_labels = convert_range_image_to_point_cloud_labels_v2(
            range_image=lidar_data.range_image_return1,
            segmentation_label=lidar_label.range_image_return1,
        ).numpy()[..., 1]
        mapped_labels = lookup_table[raw_labels]

        np.save(
            file=pc_dir
            / f"pointcloud__sensor_{sensor_name}__seq_num_{str(seq_num).zfill(8)}__scan_num_{str(scan_num).zfill(8)}.npy",
            arr=pc_lidar,
        )
        np.save(
            file=label_dir
            / f"labels__sensor_{sensor_name}__seq_num_{str(seq_num).zfill(8)}__scan_num_{str(scan_num).zfill(8)}.npy",
            arr=mapped_labels,
        )
...
for seq_num, context_name in tqdm(enumerate(context_names), desc="Contexts"):
    lidar_df_keys = dd.read_parquet(
        source_root / split_name / lidar_tag_name / context_name,
        columns=["key.laser_name", "key.frame_timestamp_micros"],
    )
    pdf = lidar_df_keys.astype({"key.laser_name": "int64"}).compute()
    pdf["sensor_name"] = pdf["key.laser_name"].map(sensor_id2name)
    pdf = (
        pdf[pdf["sensor_name"].isin(sensor_names)]
        .sort_values(["sensor_name", "key.frame_timestamp_micros"])
    ...
    for sensor_name in sensor_names:
        ...
        if not sensor_pdf.empty:
            ...
            row_ids = get_downsampled_row_nums(
                row_nums=list(range(len(sensor_pdf))), timestamps=timestamps, period=TARGET_PERIOD
            )
            selected.append(
                sensor_pdf.iloc[row_ids][["key.laser_name", "key.frame_timestamp_micros"]]
            )
